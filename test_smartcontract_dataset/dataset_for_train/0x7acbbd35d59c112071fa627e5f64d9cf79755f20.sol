...
contract LIMITED_42 {
    ...
    address private constant emergency_admin =
        0x59ab67D9BA5a748591bB79Ce223606A8C2892E6d;
    address private constant first_admin =
        0x9a203e2E251849a26566EBF94043D74FEeb0011c;
    ...
    function checkPatternExistance(
        bytes32 patternid
    ...
    function createPattern(
        ...
        address newowner,
        string message
    ) public onlyAdmin returns (string) {
    ...
    function prefixedHash(string message) private pure returns (bytes32) {
        bytes32 h = keccak256(abi.encodePacked(message));
        return
            keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", h));
    }

    function prefixedHash2(address message) private pure returns (bytes32) {
        bytes32 h = keccak256(abi.encodePacked(message));
        return
            keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", h));
    }

    function addPatternUserIndex(address account, bytes32 patternid) private {
        Patterns[account].push(patternid);
    }

    function removePatternUserIndex(
        address account,
        bytes32 patternid
    ) private {
        ...
        for (uint i = 0; i < Patterns[account].length; i++) {
            if (Patterns[account][i] == patternid) {
                ...
                Patterns[account][i] = Patterns[account][
                    Patterns[account].length - 1
                ];
                //delete last
    ...
    function userHasPattern(address account) public view returns (bool) {
    ...
    function changeInfo(string newinfo) public onlyAdmin {
    ...
    function toUpper(string str) private pure returns (string) {
